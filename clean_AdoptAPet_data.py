    ...
    for row in data:
        ...
        for dog_data in dogs_name_data:
            if row['primary_breed'] in dog_data['Breed']:
                ...
                row['Weight (lbs)'] = dog_data['Weight (lbs)']
                row['Lifespan (Yrs)'] = dog_data['Lifespan (Yrs)']
...
def clean_dogs_name_data(filename: str) -> pd.DataFrame:
    ...
    dog_data['Avg Lifespan (Yrs)'] = dog_data['Lifespan (Yrs)'].apply(parse_lifespan)
    dog_data['Avg Weight (lbs)'] = dog_data['Weight (lbs)'].apply(parse_weight)
